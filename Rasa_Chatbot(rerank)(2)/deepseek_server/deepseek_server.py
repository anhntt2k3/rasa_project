...
from memory import save_history, get_relevant_history, reset_memory

app = FastAPI()

# Reset dữ liệu khi chương trình khởi chạy
reset_memory()
...
def query_deepseek(request: QuestionRequest):
    ...
    #LangChain pipeline
    history_data = get_relevant_history(user_question)  # Lấy lịch sử trước
    print("history:", history_data)  # Debug
    chain = (
        {
            "user_question": RunnableLambda(lambda x: x["user_question"]), 
            "context": RunnableLambda(lambda x: x["context"]), 
            "history": RunnableLambda(lambda x: get_relevant_history(x["user_question"]))  # Tìm trong cả Q&A
        } 
        | prompt 
        | llm 
        | StrOutputParser()
    ...
    try:
        ...
        # Lưu vào bộ nhớ
        save_history(user_question, response)
